...
type ProviderId = "platform" | "openrouter" | "respan" | "nvidia";
...
function ProviderGlyph({ providerId }: { providerId: ProviderId }) {
  return <span className={`provider-glyph ${providerId}`}>{providerId === "platform" ? "P" : providerId === "openrouter" ? "OR" : providerId === "nvidia" ? "NV" : "R"}</span>;
}
...
export default function GodmodeChatWorkspace() {
  ...
  const [connectProvider, setConnectProvider] = useState<"openrouter" | "respan" | "nvidia" | undefined>();
  ...
  async function submitMessage(event?: FormEvent) {
    ...
    if (!models.length) { toast.error("Connect OpenRouter, NVIDIA NIM, or Respan to load currently callable models."); setSettingsOpen(true); return; }
    ...
    try {
      if (mode === "solo" && !researchMode && (selectedModels[0]?.providerId === "openrouter" || selectedModels[0]?.providerId === "nvidia")) {
        const streamingProvider = selectedModels[0].providerId;
        setIsStreaming(true); setStreamingContent(""); setStreamFirstTokenMs(null); setStreamStatus(streamingProvider === "nvidia" ? "Opening NVIDIA NIM route…" : "Opening strict free fast route…"); setStreamModelId(streamingProvider === "nvidia" ? selectedModels[0].modelId : "OpenRouter free fast route"); setStreamProviderId(streamingProvider);
  ...
  async function connect(event: FormEvent) {
    ...
    try {
      ...
      await utils.godmode.providers.list.invalidate();
      toast.success(`${connectProvider === "openrouter" ? "OpenRouter" : connectProvider === "nvidia" ? "NVIDIA NIM" : "Respan"} connected and verified.`);
    } catch (error) { toast.error(error instanceof Error ? error.message : "Provider verification failed."); }
  }

  async function disconnect(providerId: "openrouter" | "respan" | "nvidia") {
    try {
  ...
  return <main className="chat-shell">
    ...
    {connectProvider && <div className="connect-overlay"><form className="connect-card" onSubmit={connect}><button type="button" className="connect-close" onClick={() => { setConnectProvider(undefined); setApiKey(""); }}><X size={18} /></button><ProviderGlyph providerId={connectProvider} /><p>{connectProvider === "openrouter" ? "OPENROUTER" : connectProvider === "nvidia" ? "NVIDIA NIM" : "RESPAN"} CONNECTION</p><h2>Connect an API key.</h2><span>{connectProvider === "nvidia" ? "Your NVIDIA NIM key is encrypted server-side and verified against its live model catalog. NVIDIA describes this route as development/prototyping access; limits can still apply." : "The key is sent only to the server, verified with a live model request, and encrypted before persistence. It never returns to this browser."}</span><label>API KEY<input autoFocus type="password" value={apiKey} onChange={event => setApiKey(event.target.value)} placeholder={connectProvider === "openrouter" ? "sk-or-v1-…" : connectProvider === "nvidia" ? "nvapi-…" : "Paste Respan API key"} /></label><button className="signal-primary" disabled={!apiKey.trim() || attachProvider.isPending}>{attachProvider.isPending ? <Loader2 className="spin" size={16} /> : <KeyRound size={16} />}Verify and connect</button></form></div>}
